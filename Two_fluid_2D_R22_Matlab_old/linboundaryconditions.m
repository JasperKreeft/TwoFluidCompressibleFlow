function [FBC,GBC]=linboundaryconditions(NX,NY,W,Wfx,Wfy,gamma1,gamma2)
...
if (BC_L==5) 
    for i=1:NY
        ...
        FBC(i,1,2) = rhoflux*uflux^(2e0)+pflux;
        FBC(i,1,3) = rhoflux*uflux*vflux;
        FBC(i,1,4) = (alphaflux/(gamma1-1e0)+(1e0-alphaflux)/(gamma2-1e0)+1e0)*uflux*pflux+0.5e0*rhoflux*uflux*(uflux^(2e0)+vflux^(2e0));
        FBC(i,1,5) = betaflux*rhoflux*uflux;
        FBC(i,1,6) = gamma1/(gamma1-1e0)*uflux*pflux*alphaflux+1e0/2e0*rhoflux*uflux*(uflux^(2e0)+vflux^(2e0))*betaflux;
    end
end
...
if (BC_R==5)
    for i=1:NY
        ...
        FBC(i,NX+1,2) = rhoflux*uflux^(2e0)+pflux;
        FBC(i,NX+1,3) = rhoflux*uflux*vflux;
        FBC(i,NX+1,4) = (alphaflux/(gamma1-1e0)+(1e0-alphaflux)/(gamma2-1e0)+1e0)*uflux*pflux+0.5e0*rhoflux*uflux*(uflux^(2e0)+vflux^(2e0));
        FBC(i,NX+1,5) = betaflux*rhoflux*uflux;
        FBC(i,NX+1,6) = gamma1/(gamma1-1e0)*uflux*pflux*alphaflux+1e0/2e0*rhoflux*uflux*(uflux^(2e0)+vflux^(2e0))*betaflux;
    end
end
...
if (BC_A==5) 
    for j=1:NX
        ...
        GBC(1,j,3) = rhoflux*vflux^(2e0)+pflux;
        GBC(1,j,4) = (alphaflux/(gamma1-1e0)+(1e0-alphaflux)/(gamma2-1e0)+1e0)*vflux*pflux+0.5e0*rhoflux*vflux*(uflux^(2e0)+vflux^(2e0));
        GBC(1,j,5) = betaflux*rhoflux*vflux;
        GBC(1,j,6) = gamma1/(gamma1-1e0)*vflux*pflux*alphaflux+1e0/2e0*rhoflux*vflux*(uflux^(2e0)+vflux^(2e0))*betaflux;
    end
end
...
if (BC_B==5) 
    for j=1:NX
        ...
        GBC(NY+1,j,3) = rhoflux*vflux^(2e0)+pflux;
        GBC(NY+1,j,4) = (alphaflux/(gamma1-1e0)+(1e0-alphaflux)/(gamma2-1e0)+1e0)*vflux*pflux+0.5e0*rhoflux*vflux*(uflux^(2e0)+vflux^(2e0));
        GBC(NY+1,j,5) = betaflux*rhoflux*vflux;
        GBC(NY+1,j,6) = gamma1/(gamma1-1e0)*vflux*pflux*alphaflux+1e0/2e0*rhoflux*vflux*(uflux^(2e0)+vflux^(2e0))*betaflux;
    end
end
