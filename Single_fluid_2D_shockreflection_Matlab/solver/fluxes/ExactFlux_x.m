% Exact flux solver for the x-direction

function [F,Lambda] = ExactFlux_x(NX,NY,Wfx,gamma,Lambda)
        ...
        elseif M1<-1 && M4<-1
            ...
            if p1>p01 && p4>p01 %2 expansion waves (exact solution possible)
                ...
                while fout>1e-8
                    ...
                    H = (m1*p4+m4*p1-m1*m4*(u4-u1))/(m1+m4+eps) - p;
            ...
            F(i,j,:) = [rhof*uf;
                        rhof*uf^2+pf;
                        rhof*uf*vf;
                        uf*pf*gamma/(gamma-1)+1/2*rhof*uf*(uf^2+vf^2)];
