...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [F,sL,sR,Lambda,speeds] = LinearOsherFlux(N,Wf,gamma1,gamma2,pi1,pi2,Lambda)
...
for i  = 2:N
    ...
    % Two-fluid problem
        ...
        %Sonic point va1ues
        [rhosl,usl,psl,betasl,alphasl]=linosher_sonic_L(rho1,u1,p1,beta1,alpha1,a1,gamma1,gamma2,pi1,pi2);
        [rhosr,usr,psr,betasr,alphasr]=linosher_sonic_R(rho4,u4,p4,beta4,alpha4,a4,gamma1,gamma2,pi1,pi2);
        ...
        speeds(i,:) = [lambda0 lambda1 lambda2 lambda3 lambda4];
        
        if lambda0 >= 0 && lambda2>=0
            if lambda1>=0 && lambda4>=0
                F(:,i) = F1;
%                 [sint12] = Sint_lin(rho1,u1,p1,beta1,alpha1,u23,gamma1,gamma2,pi1,pi2,-1);
%                 [sintCD] = SintCD(u23,p23,alpha3,alpha2);
%                 [sint34] = Sint_lin(rho4,u4,p4,beta4,alpha4,u23,gamma1,gamma2,pi1,pi2,+1);
%                 sL(i-1) = 0;
%                 sR(i)   = sint12+sintCD-sint34;
            elseif lambda1>=0 && lambda4<=0
                F(:,i) = F1+F4-Fsr;
%                 [sintsr4] = Sint_lin(rho4,u4,p4,beta4,alpha4,usr,gamma1,gamma2,pi1,pi2,+1);
%                 [sint12]  = Sint_lin(rho1,u1,p1,beta1,alpha1,u23,gamma1,gamma2,pi1,pi2,-1);
%                 [sintCD]  = SintCD(u23,p23,alpha3,alpha2);
%                 [sint3sr] = Sint_lin(rhosr,usr,psr,betasr,alphasr,u23,gamma1,gamma2,pi1,pi2,+1);
%                 sL(i-1) = -sintsr4;
%                 sR(i)   = sint12+sintCD-sint3sr;
            elseif lambda1<=0 && lambda4>=0
                F(:,i) = Fsl;
%                 [sint1sl] = Sint_lin(rho1,u1,p1,beta1,alpha1,usl,gamma1,gamma2,pi1,pi2,-1);
%                 [sintsl2] = Sint_lin(rhosl,usl,psl,betasl,alphasl,u23,gamma1,gamma2,pi1,pi2,-1);
%                 [sintCD]  = SintCD(u23,p23,alpha3,alpha2);
%                 [sint34]  = Sint_lin(rho4,u4,p4,beta4,alpha4,u23,gamma1,gamma2,pi1,pi2,+1);
%                 sL(i-1) = sint1sl;
...
% Left sonic point                                                        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rhosl,usl,psl,betasl,alphasl]=linosher_sonic_L(rho1,u1,p1,beta1,alpha1,a1,gamma1,gamma2,pi1,pi2)
...
% Right sonic point                                                       %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rhosr,usr,psr,betasr,alphasr]=linosher_sonic_R(rho4,u4,p4,beta4,alpha4,a4,gamma1,gamma2,pi1,pi2)
