...
%     % Single-fluid Osher
%     elseif ((alpha1<1e-10 && alpha4<1e-10) || ((1-alpha1)<1e-10 && (1-alpha4)<1e-10)) ...
%            && ((beta1<1e-10 && beta4<1e-10) || ((1-beta1)<1e-10 && (1-beta4)<1e-10)) % single fluid problem
%         
...
%                 [sint34] = Sint(rho3,u23,p23,beta3,alpha3,u4,gamma1,gamma2,pi1,pi2,+1);
                [sint34] = Sint(rho4,u4,p4,beta4,alpha4,u23,gamma1,gamma2,pi1,pi2,+1);
                sL(i-1) = 0;
                sR(i)   = sint12+sintCD-sint34;
            elseif lambda1>=0 && lambda4<=0
                F(:,i) = Fl+Fr-Fsr;
%                 [sintsr4] = Sint(rhosr,usr,psr,betasr,alphasr,u4,gamma1,gamma2,pi1,pi2,+1);
                [sintsr4] = Sint(rho4,u4,p4,beta4,alpha4,usr,gamma1,gamma2,pi1,pi2,+1);
                [sint12]  = Sint(rho1,u1,p1,beta1,alpha1,u23,gamma1,gamma2,pi1,pi2,-1);
                [sintCD]  = SintCD(u23,p23,alpha3,alpha2);
%                 [sint3sr] = Sint(rho3,u23,p23,beta3,alpha3,usr,gamma1,gamma2,pi1,pi2,+1);
                [sint3sr] = Sint(rhosr,usr,psr,betasr,alphasr,u23,gamma1,gamma2,pi1,pi2,+1);
                sL(i-1) = -sintsr4;
...
%                 [sint3sr] = Sint(rho3,u23,p23,beta3,alpha3,usr,gamma1,gamma2,pi1,pi2,+1);
            ...
            elseif lambda1<=0 && lambda4>=0
                ...
                [sint12] = Sint(rho1,u1,p1,beta1,alpha1,u23,gamma1,gamma2,pi1,pi2,-1);
                [sintCD] = SintCD(u23,p23,alpha3,alpha2);
%                 [sint34_1] = Sint(rho3,u23,p23,beta3,alpha3,u4,gamma1,gamma2,pi1,pi2,+1);
                [sint34] = Sint(rho4,u4,p4,beta4,alpha4,u23,gamma1,gamma2,pi1,pi2,+1);
                sL(i-1) = sint12;
                sR(i)   = sintCD-sint34;
%                 sint34_1-sint34
...
%         if abs(sL(i-1))>0.99 && abs(sL(i-1))<1.01; keyboard; end
%         if abs(sR(i))>0.99 && abs(sR(i))<1.01; keyboard; end
    end %if L==R
end %for i loop

% if source==true; keyboard; end
...
while diffp>1e-3
    ...
    for k=1:NRK4
        ...
        [dp_k4,drho_k4,dalpha_k4]=RK4_p(rho,p,alpha,2*drho_k3,2*dp_k3,2*dalpha_k3,gamma1,gamma2,pi1,pi2,-dul);
...
% keyboard
    ...
    for k=1:NRK4
        ...
        [dp_k4,drho_k4,dalpha_k4]=RK4_p(rho,p,alpha,2*drho_k3,2*dp_k3,2*dalpha_k3,gamma1,gamma2,pi1,pi2,dur);
        p = p + (dp_k1 + 2*dp_k2 + 2*dp_k3 + dp_k4)/6;
        rho = rho + (drho_k1 + 2*drho_k2 + 2*drho_k3 + drho_k4)/6;
        alpha = alpha + (dalpha_k1 + 2*dalpha_k2 + 2*dalpha_k3 + dalpha_k4)/6;
    end
