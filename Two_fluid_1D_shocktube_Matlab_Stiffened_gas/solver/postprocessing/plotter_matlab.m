...
rho1   = W(1,1,end);%rho(1);
u1     = W(2,1,end);%u(1);
p1     = W(3,1,end);%p(1);

rho4   = W(1,end,end);%rho(end);
u4     = W(2,end,end);%u(end);
p4     = W(3,end,end);%p(end);
...
if pf<p1
for j=ceil((x1+0.5)/dx):floor((x2+0.5)/dx)
    xexp          = j*dx;
...
for j=ceil((x2+0.5)/dx):floor((x3+0.5)/dx)
    rhoexact(j)   = rho2;
    uexact(j)     = uf;
    pexact(j)     = pf;
    betaexact(j)  = 1;
...
for j=ceil((x3+0.5)/dx):floor((x4+0.5)/dx)
    rhoexact(j)   = rho3;
    uexact(j)     = uf;
    pexact(j)     = pf;
    betaexact(j)  = 0;
...
if pf<p4
for j=ceil((x4+0.5)/dx):floor((x5+0.5)/dx)
    xexp          = j*dx-0.5;
...
plot(x,rho,'xg','MarkerSize',5,'linewidth',1)
% set(gca,'fontsize',14)
% grid on
% xlim([0 1])
% ylim([0.2 1.2])
...
plot(x,u,'xg','MarkerSize',5,'linewidth',1)
% set(gca,'fontsize',14)
% grid on 
% xlim([0 1])
% ylim([-0.2 2])
...
plot(x,p,'xg','MarkerSize',5,'linewidth',1)
% set(gca,'fontsize',14)
% grid on
% xlim([0 1])
% ylim([0 11])
...
plot(x,alpha,'xg','MarkerSize',5,'linewidth',1)
% set(gca,'fontsize',14)
...
plot(x,beta,'xg','MarkerSize',5,'linewidth',1)
% set(gca,'fontsize',14)
% xlim([0.5 1])
ylim([-0.2 1.2])
xlabel('x')
