...
function [F,Lambda] = OsherFlux_generalEOS(N,Wf,gamma,Lambda)
...
for i  = 2:N
    ...
    u1      = Wf(2,2*i-2);
    p1      = Wf(3,2*i-2);
    [e1,a1] = EOS(rho1,p1,gamma);
    M1      = u1/a1;

    rho4    = Wf(1,2*i-1);
    u4      = Wf(2,2*i-1);
    p4      = Wf(3,2*i-1);
    [e4,a4] = EOS(rho4,p4,gamma);
    M4      = u4/a4;
    ...
    % cavity test
    if (u1+2/(gamma-1)*a1)<(u4-2/(gamma-1)*a4) % afhankelijk van EOS
    ...
    if abs(rho1-rho4)<1e-12 && abs(u1-u4)<1e-12 && abs(p1-p4)<1e-12
        ...
        F(:,i) = [rho1*u1;
                  rho1*u1^2+p1;
                  rho1*e1*u1+p1*u1+1/2*rho1*u1^3];
    ...
    elseif M1<-1 && M4<-1
        ...
        F(:,i) = [rho4*u4;
                  rho4*u4^2+p4;
                  rho4*e4*u4+p4*u4+1/2*rho4*u4^3];
        ...
        % Speed of sound
        [e2,a2] = EOS(rho2,p23,gamma);
        [e3,a3] = EOS(rho3,p23,gamma);

        % Sonic point va1ues
        
        % moet nog: dp/rhoa als functie van rho,u,p,gamma.
        
%         usl   = u1 \pm dp/rhoa;
        usl   = (gamma-1)/(gamma+1)*u1+2*a1/(gamma+1);
        asl   = usl;
        rhosl = rho1*(asl/a1)^(2/(gamma-1)); % isentrope relatie afhankelijk van EOS
        psl   = p1*(rhosl/rho1)^gamma;       % isentrope relatie afhankelijk van EOS

%         usr   = u1 \pm dp/rhoa;
        usr   = (gamma-1)/(gamma+1)*u4-2*a4/(gamma+1);
        asr   = -usr;
        rhosr = rho4*(asr/a4)^(2/(gamma-1)); % isentrope relatie afhankelijk van EOS
        psr   = p4*(rhosr/rho4)^gamma;       % isentrope relatie afhankelijk van EOS

        F1  = [   rho1*u1;    rho1*u1^2+p1;            rho1*e1*u1+p1*u1+0.5*rho1*u1^3];
        F2  = [  rho2*u23;  rho2*u23^2+p23;        rho2*e2*u23+p23*u23+0.5*rho2*u23^3];
        F3  = [  rho3*u23;  rho3*u23^2+p23;        rho3*e3*u23+p23*u23+0.5*rho3*u23^3];
        F4  = [   rho4*u4;    rho4*u4^2+p4;            rho4*e4*u4+p4*u4+0.5*rho4*u4^3];
