...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
% Exact solution and L_1, L_2 and L_infty error calculations              %
% for Single and Two Fluid Flow Shocktube Solver.                         %
%                                                                         %
...
% left wave state
if pf<p1
    for j=ceil(x1/dx+N/2):ceil(x2/dx+N/2)
        xexp          = j*dx-dx/2;
...
%  final state 2
for j=ceil(x2/dx+N/2):ceil(x3/dx+N/2)
    rhoexact(j)   = rho2;
    uexact(j)     = uf;
    pexact(j)     = pf;
    betaexact(j)  = beta1;
...
if pf<p4
    for j=ceil(x4/dx+N/2):floor(x5/dx+N/2)
        xexp          = j*dx-dx/2;
